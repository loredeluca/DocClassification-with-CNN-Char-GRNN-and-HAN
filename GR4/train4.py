...
from prep4 import GRNNDataset, CustomDataloader, load_word2vec_embeddings
from model4 import DocSenModel
...
def train(data_folder, n_classes, word_map, sentence_model, gnn_type, gnn_output, model_name, early_stopping=2):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    batch_size = 50
    training_set = GRNNDataset(data_folder, 'train')
    dataloader_train = CustomDataloader(batch_size, training_set)

    valid_set = GRNNDataset(data_folder, 'val')
    val_indices = [*range(0, valid_set.shape[0], 1)]
    dataloader_valid = CustomDataloader(batch_size, valid_set)
    ...
    model = DocSenModel(n_classes, sentence_model, gnn_output, gnn_type, embeddings, device=device, freeze_embedding=True)
