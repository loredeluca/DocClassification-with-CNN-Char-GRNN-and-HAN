...
from utils3 import HANDataset, AverageMeter
...
def test():
    ...
    # Load test data
    test_loader = DataLoader(HANDataset(data_folder, 'test'), batch_size=64, shuffle=False, num_workers=4, pin_memory=True)
    ...
    for i, (documents, sentences_per_document, words_per_sentence, labels) in enumerate(tqdm(test_loader, desc='Evaluating')):
        ...
        start = time.time() # non penso serva
